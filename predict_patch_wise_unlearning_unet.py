        ...
        for z in z_starts:
            for y in y_starts:
                for x in x_starts:
                    ...
                    patch_src = model._normalize(patch_src)  # [-1, 1]
                    
                    # Prédiction
                    feature_map = model.feature_extractor(patch_src)
                    patch_pred = model.model.forward(feature_map, t=None)

                    # Reconstruction inverse
                    patch_pred = model._denormalize(patch_pred)  # [0, SUV_LOG_MAX]

                    # Accumulation
                    output_volume[z:z + z_patch_size, y:y + y_patch_size, x:x + x_patch_size] += patch_pred.squeeze(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--filename', '-f', type=str, required=False, default='harmonized_PET_unet', help='Filename to process (if needed).')
