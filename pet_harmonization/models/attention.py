...
class Attention(nn.Module):
    ...
    def forward(self, x, temb=None):
        if hasattr(self, 'attention'):
            return self.attention(x, temb)
        else:
            return x 
        
    def __repr__(self):
        return f"{self.__class__.__name__}(type={self.attention_type}, num_heads={self.num_heads}, channels_per_head={self.ch_per_head})"