...
class BasicNoiseScheduler(nn.Module):
    ...
    def sample(self, x_0, scale=None):
        ...
        return self.estimate_x_t(x_0, t, x_T, scale=scale), x_T, t
    ...
    def estimate_x_t(self, x_0, t, x_T=None, scale=None, **kwargs):
        """Get x_t at time t"""
        raise NotImplemented
...
class GaussianNoiseScheduler(BasicNoiseScheduler):
    ...
    def estimate_x_t(self, x_0, t, x_T=None, scale=None):
        ...
        def clipper(b):
            tb = t[b]
            sb = scale[b] if scale is not None else torch.tensor(1.0)
            if tb < 0:
                return x_0[b]
            elif tb >= self.T:
                return x_T[b] * torch.sqrt(sb)
            else:
                x_0[b] *= self.sqrt_alphas_cumprod[tb]
                x_0[b] += self.sqrt_one_minus_alphas_cumprod[tb] * x_T[b] * torch.sqrt(sb)
                return x_0[b]
